...
class GamePAI():
    ...
    def playerAction(self,action):
        ...
        self.calculate_reward(action,bool_player_move,attack_boolean,kill_boolean,unknownTille,weaponPicked,oldhitpoints,oldmanapoints,enter_cave_boolean)
    ...
    def calculate_reward(self,action,bool_player_move,attack_boolean,kill_boolean,unknownTille,weaponPicked,oldhitpoints,oldmanapoints,enter_cave_boolean):
        reward = 0
        if action <= 3:
            ...
            if not bool_player_move:
                reward = -1 # self.reward_useless_action
        ...
        if action == 6:
            if attack_boolean:
                    reward = self.reward_usefull_action*10
                ...
                if settings.tiles[i][h].visibility == GameEnum.VisibilityType.visible and settings.tiles[i][h].store != None:
                    if isinstance(self.player,Wizard):
                        if isinstance(settings.tiles[i][h].store, ManaPotion) and settings.tiles[i][h].store.seen != True:
                            ...
                            self.reward +=100
                        if isinstance(settings.tiles[i][h].store, Staff) and settings.tiles[i][h].store.seen != True:
                            settings.tiles[i][h].store.seen = True
                            self.reward +=300
                    if isinstance(settings.tiles[i][h].store, HelthPotion) and settings.tiles[i][h].store.seen != True:
                        ...
                        self.reward +=100
                    if isinstance(self.player,Warrior):
                        if isinstance(settings.tiles[i][h].store, Sword) and settings.tiles[i][h].store.seen != True:
                            settings.tiles[i][h].store.seen = True 
                            self.reward +=300
