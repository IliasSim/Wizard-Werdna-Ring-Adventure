...
class GamePAI():
    ...
    def playerAction(self,action):
        ...
        if action <= 3:
            ...
            if Xposition != self.player.currentPositionX or Yposition != self.player.currentPositionY:
                ...
                self.enemyMove()
                enter_cave_boolean,end_game =self.enterCave(self.cave)
        ...
        state = self.create_state_for_nn()
        ...
        if self.player.weapon != None:
            ...
            weaponencode = 0
        playerstatus = np.array([weaponencode,self.player.hitPoints/self.player.maxHitPoints,self.player.maxHitPoints/100,self.player.getAttackDamage()/45,self.countHealthPotion()/30,self.player.getLevel()/5,self.player.experiencePoints/14000,self.cave/10],dtype=np.float16)
        ...
        if self.player.hitPoints <= 0 or end_game == True:
            done = True
        ...
        playerstatus = playerstatus.astype('float16')
        textArray = textArray.astype('float16')
    ...
    def gameVocab(self,textList):
        ...
        'Amazing':90,'Deadly':91,'Ancient':92,'Sword':93,'Staff':94,'weapon.':95,'rest':96,'Ettin':97,"Werdna's":98,'end':99,'game.':100}
    ...
    def enterCave(self,cave):
        ...
        if isinstance(settings.tiles[self.player.currentPositionX][self.player.currentPositionY].store, Werdna_Ring):
            ...
            end_game = True
            #sys.exit()
        return enter_cave_boolean,end_game
    ...
    def create_state_for_nn(self):
        state = np.zeros((5,settings.xtile,settings.ytile))
        for x in range(settings.xtile):
            for y in range(settings.ytile):
                if self.player.currentPositionX == x and self.player.currentPositionY == y:
                    state[0][x][y] = 1
                ...
                if settings.tiles[x][y].visibility == GameEnum.VisibilityType.visible:
                    ...
                    if settings.enemies != []:
                        for enemy in settings.enemies:
                            if enemy.enemyCurrentPossitionX==x and enemy.enemyCurrentPossitionY == y:
                ...
                # print(state[0][x][y],state[1][x][y],state[2][x][y],state[3][x][y],state[4][x][y])   
        return state
                    
                    
                    
if __name__ == '__main__': 
    game = GamePAI(1,'Connan',444,444,1,True,0,True,False,0)
