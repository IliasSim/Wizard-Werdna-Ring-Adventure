...
class Agent():
    ...
    def learn(self, states,playerstatus,gameTexts, discnt_rewards,actions,global_actor,global_critic):
...
def runner(barrier,total_episode,agent_i,queue_actor,queue_critic,lock):
    # tf.random.set_seed(39999)
        global_actor = actor()
        global_critic = critic()
        global_actor(tf.convert_to_tensor(np.zeros((input1)), dtype=tf.float32),
                            tf.convert_to_tensor(np.zeros((input2)), dtype=tf.float32),
                            tf.convert_to_tensor(np.zeros((input3)), dtype=tf.float32))
        global_critic(tf.convert_to_tensor(np.zeros((input1)), dtype=tf.float32),
                            tf.convert_to_tensor(np.zeros((input2)), dtype=tf.float32),
                            tf.convert_to_tensor(np.zeros((input3)), dtype=tf.float32))
        ...
        dir = 'D:\ekpa\diplomatiki\Wizard-Werdna-Ring-Adventure\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\episodes_agent' + str(agent_i) + '.txt'
        if not exists(dir):
            f = open(dir,'w+')
            f.write('1')
        f = open(dir,'r')
        ...
        for s in range(episodes_text,episode):
            ...
            while not done:
                ...
                # print(agent_i,action_name[action],reward,game.cave,steps,s,len(replay_memory.replay_buffer))
                total_reward += reward
                state = next_state
                ...
                if steps > truncate + h_step - 1:
                    if steps%truncate == 0 or done:
                        ...
                        global_critic.set_weights(queue_critic)
                        for i in range(len(train_states)):
                            al,cl = agent.learn(train_states[i],train_playerstatus[i],train_gametexts[i], discounted_rewards_all[i], train_actions[i],global_actor,global_critic)
                ...
                if done:
                    ...
                    total_episode[0] += 1
                    # lock.release()
                    print('total episode' + str(total_episode[0]))
                    if total_episode[0]%5 == 0 and total_episode[0] !=0:
                        print('do i work')
                        global_actor.save_weights('.\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\ actor_model2')
                        global_actor.save_weights('.\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\ critic_model2')
                        f = open("D:\ekpa\diplomatiki\Wizard-Werdna-Ring-Adventure\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\ agent.txt",'w')
                        f.write(str(agent_i))
                        f.close()
                        f = open("D:\ekpa\diplomatiki\Wizard-Werdna-Ring-Adventure\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\ episodes.txt",'w')
                        f.write(str(total_episode[0]))
                        f.close()
...
if __name__ == '__main__':
    ...
    if exists('D:\ekpa\diplomatiki\Wizard-Werdna-Ring-Adventure\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\ critic_model2.data-00000-of-00001'):
        ...
        global_critic.load_weights('D:\ekpa\diplomatiki\Wizard-Werdna-Ring-Adventure\modelLSTM_Imp_15_12_21_nreplay_n_step_truncate_multiple_agents\ critic_model2')
    ...
    processes = []
    for i in range(4):
        worker = mp.Process(target = runner, args=(barrier,total_episodes,i,queue_actor,queue_critic,lock))
