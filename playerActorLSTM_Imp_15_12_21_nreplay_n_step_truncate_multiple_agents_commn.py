...
class Agent():
    ...
    def __init__(self,agent_i):
        ...
        self.a_c_opt = tf.keras.optimizers.Adam(learning_rate=0.0001)
        self.log_prob = None
        self.buffer_State = []
...
def runner(total_episode,agent_i,queue_actor_critic,manager_grades_a_c,barrier):
    # tf.random.set_seed(39999)
        agent = Agent(agent_i)
        agent.local_actor_critic.set_weights(queue_actor_critic)
        ...
        for s in range(episodes_text,episode):
            ...
            steps = 0
            agent.local_actor_critic.set_weights(queue_actor_critic)
            # total_agent_actor_critic_grads = [tf.zeros_like(this_var) for this_var in global_actor_critic.trainable_variables]
            while not done:
                # if steps%truncate == 0:
                    # agent.local_actor_critic.set_weights(queue_actor_critic)
                ...
                if steps > truncate + h_step - 1:
                    if steps%truncate == 0 or done:
                        ...
                        loss,local_grads = agent.learn(train_states,train_playerstatus,train_gametexts, discounted_rewards, train_actions)
                        print('agent ' +  str(agent_i) + ' Actor_Critic Losses ' + str(loss))
                        lock.aquire()
                        manager_grades_a_c = [(acum_grad+grad) for acum_grad, grad in zip(manager_grades_a_c, local_grads)]
                        lock.release()
                        barrier.wait()
                        agent.a_c_opt.apply_gradients(zip(manager_grades_a_c, agent.local_actor_critic.trainable_variables))
...
if __name__ == '__main__':
    ...
    global_actor_critic(tf.convert_to_tensor(np.zeros((input1)), dtype=tf.float32),
    ...
    episode_dict = {}
    agent_actor_critic_grads = [tf.zeros_like(this_var) for this_var in global_actor_critic.trainable_variables]
    ...
    manager_grades_a_c =  mp.Manager().list(agent_actor_critic_grads)
    barrier = mp.Barrier(mp.cpu_count())
    # grads_actor_critic = mp.Manager().list(actor_critic_accum_gradient)
    processes = []
    for i in range(mp.cpu_count()):
        worker = mp.Process(target = runner, args=(total_episode,i,manager_actor_critic,manager_grades_a_c,barrier,))
