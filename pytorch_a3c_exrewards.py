...
from GamePAI_pytorch_dqn import GamePAI
import sys
import pygame
import numpy
import copy
...
class Sequence_Buffer:
    ...
    def TakeSequence(self):
        ...
        for i in range(len(self.buffer)):
            if len(self.buffer) == truncate:
                last_value_state = self.buffer[-1][4]
            if len(self.buffer) < truncate:
                last_value_state = 0
            disc_reward = 0
            power = 0
            for z in range(i,len(self.buffer)):
                ...
                power += 1
            disc_reward = disc_reward + last_value_state*gamma**power
        ...
        while not done:
            ...
            if done:
                ...
                if episodes[0]%100==0 and episodes[0] != 0:
                    torch.save(global_model.state_dict(), 'D:\ekpa\diplomatiki\date_18_5_22_exrewards\gen_model.pt')
                    d = dfrewards
                    d = list(d)
                    ...
                    destination = r'D:\ekpa\diplomatiki\date_18_5_22_exrewards\agent_' + str(agent) + '.xlsx'
                ...
                if episodes[0] == total_episodes + mp.cpu_count():
                    ...
                    # print(type(d))
                    dfrewards_data_frame = pd.DataFrame(d, columns=['agent','agent episode','episode rewards', 'total agent rewards', 'mean agent rewards','total episodes','total reward','total average reward'])
                    destination = r'D:\ekpa\diplomatiki\date_18_5_22_exrewards\agent_.xlsx'
                    dfrewards_data_frame.to_excel(destination)
                    process_cont = False
...
if __name__ == '__main__':
    ...
    if exists('D:\ekpa\diplomatiki\date_18_5_22_exrewards\gen_model.pt'):
        global_model.load_state_dict(torch.load('D:\ekpa\diplomatiki\date_18_5_22_exrewards\gen_model.pt'))
        global_model.eval()
