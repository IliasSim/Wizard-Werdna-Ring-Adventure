...
def Agent_Runner():
    ...
    # if agent == 2:
    # optimizer =  torch.optim.Adam(dqn_net.parameters(), lr=0.01) # 0.00001 for breakout, 0.00025 is faster for pong
    # if agent == 3:
    ...
    while training_cont: 
        ...
        for i in range(len(sequence)):
            ...
            if pseudo_steps in episode_list:
                ...
                print(loss.item(),action_name[torch.argmax(q_value).item()],action_name[sequence[i][7]],action_name[torch.argmax(q_value).item()]==action_name[sequence[i][7]])                 
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        average_loss = total_loss/train_epoch
        ...
        if train_epoch%(target_network_update*8) == 0:
            ...
            torch.save(dqn_net.state_dict(), 'D:\ekpa\diplomatiki\date_31_5_22_test_5_dqn\gen_model_'+ str(train_epoch) + '.pt')                   
