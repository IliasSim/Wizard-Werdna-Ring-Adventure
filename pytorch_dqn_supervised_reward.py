...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.Q_value = nn.Linear(hiden_layer1+hiden_layer2+hiden_layer3+1,1)
        

    
    def forward(self,input1,input2,input3,inputAct,hiddenCnn = None,hiddenl1 = None,hiddenl2 = None):
        ...
        q_values = self.Q_value(torch.cat((lstmCnn_out,lstml1_out,lstml2_out,inputAct),2))
...
def Agent_Runner():
    ...
    # optimizer = torch.optim.RMSprop(dqn_net.parameters(), lr=1e-3, alpha=0.95, momentum=0.95, eps=1e-2)
    ...
    # if agent == 2:
    optimizer =  torch.optim.Adam(dqn_net.parameters(), lr=0.0001) # 0.00001 for breakout, 0.00025 is faster for pong
    # if agent == 3:
    ...
    while training_cont: 
        ...
        for i in range(len(sequence)):
            ...
            reward,next_h_out,next_hl1_out,next_hl2_out = dqn_net.forward(torch.tensor(sequence[i][0]).float()[None, None, :].cuda(),torch.tensor(sequence[i][1]).float()[None, None, :].cuda(), torch.tensor(sequence[i][2]).float()[None, None, :].cuda(),torch.tensor([sequence[i][7]]).int()[None, None, :].cuda(),h_out,hl1_out,hl2_out)
            # arget_q_value,_,_,_ = dqn_net_target.forward(torch.tensor(sequence[i][3]).float()[None, None, :].cuda(),torch.tensor(sequence[i][4]).float()[None, None, :].cuda(), torch.tensor(sequence[i][5]).float()[None, None, :].cuda(),next_h_out,next_hl1_out,next_hl2_out)
            ...
            if pseudo_steps in episode_list:
                loss = (reward-sequence[i][6])**2
                total_loss += loss
                average_loss = total_loss/train_epoch
                print(loss.item(),reward,sequence[i][6])                 
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        average_loss = total_loss/train_epoch
        ...
        if train_epoch%(target_network_update*8) == 0:
            ...
            torch.save(dqn_net.state_dict(), 'D:\ekpa\diplomatiki\date_31_5_22_test_5_dqn\gen_model_reward_'+ str(train_epoch) + '.pt')                   
