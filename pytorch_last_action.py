...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.lstmCnn = nn.LSTM(14400,hiden_layer1,batch_first=True)
        ...
        while not done:
            ...
            if steps%truncate==0 and steps!=0 or done:
                ...
                if len(states) !=0:
                    ...
                    for i in range(len(states)):
                        ...
                        # print(a_t,v_t,disc_rewards[i],actions[i],a_t[0][-1],actions[i][-1],a_t[0][-1][actions[i][-1]])
                        td = torch.sub(disc_rewards[i][-1],v_t[0][-1])
                        td_sqr = torch.pow(td,2)
                        prob_log = torch.log(a_t[0][-1])
                        action_log = torch.log(a_t[0][-1][actions[i][-1]])
                        policy_losses =  torch.mul(action_log,td)
                        entropy_loss = torch.sum(torch.mul(a_t[0][-1],prob_log))
                        entropy_loss = torch.neg(entropy_loss)
                        # print(td,td_sqr,policy_losses,prob_log,entropy_loss)
            ...
            if done:
                ...
                if episodes[0]%100==0 and episodes[0] != 0:
                    torch.save(global_model.state_dict(), 'D:\ekpa\diplomatiki\date_9_4_22_test3_nrest_last_action\gen_model.pt')
                    d = dfrewards
                    d = list(d)
                    ...
                    destination = r'D:\ekpa\diplomatiki\date_9_4_22_test3_nrest_last_action\agent_' + str(agent) + '.xlsx'
...
if __name__ == '__main__':
    ...
    if exists('D:\ekpa\diplomatiki\date_9_4_22_test3_nrest_last_action\gen_model.pt'):
        global_model.load_state_dict(torch.load('D:\ekpa\diplomatiki\date_9_4_22_test3_nrest_last_action\gen_model.pt'))
        global_model.eval()
